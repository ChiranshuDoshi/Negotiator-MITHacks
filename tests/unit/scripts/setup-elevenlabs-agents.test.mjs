...
import { AGENT_ENV_KEYS, NEGOTIATOR_CLIENT_TOOLS, NEGOTIATOR_FIRST_MESSAGE, buildAgentSpecs, buildNegotiatorPrompt, permissionErrorMessage, provisionAgents, upsertAgentIds } from "../../../scripts/setup-elevenlabs-agents.mjs";
...
describe("ElevenLabs agent provisioning", () => {
  it("builds two private agents with strict negotiation safety rules", () => {
    ...
    expect(prompt).toMatch(/synthetic and non-binding.*Do not volunteer that status/is);
  ...
  it("defaults to a read-only plan and updates existing names idempotently", async () => {
    ...
    expect(sdk.conversationalAi.agents.create).toHaveBeenCalledTimes(1);
  });
  ...
  it("preserves unrelated env lines while upserting only non-secret IDs", async () => {
    ...
    await upsertAgentIds(path, { "PolicyScout Voice Smoke": { agentId: "voice" }, "PolicyScout Negotiator": { agentId: "negotiator" } });
